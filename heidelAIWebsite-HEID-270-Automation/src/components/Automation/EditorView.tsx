    ...
    >
      <div className="p-3.5 flex items-center justify-between gap-2.5 border-b border-slate-100/80">
        <div className="flex items-center gap-2.5 min-w-0 flex-1">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-[13px] font-semibold text-slate-900 leading-tight mb-0.5 truncate">{node.label}</p>
            <p className="text-[11px] text-slate-600 leading-tight truncate">{node.description || "Configure step"}</p>
            {node.type === "trigger" && (node.data?.triggerSubtitle as string) && (
              <p className="text-[10px] text-slate-500 mt-1 truncate">{node.data.triggerSubtitle as string}</p>
            )}
          </div>
        </div>
        {isSelected && (
          <button onClick={(e) => { e.stopPropagation(); onDelete(); }} className="text-slate-300 hover:text-red-500 shrink-0 transition-colors p-1 rounded hover:bg-red-50">
      ...
      {isTrigger ? (
        <div className="absolute left-1/2 -translate-x-1/2 -bottom-3 flex flex-col items-center group z-10">
          <button onClick={onAddLink} className="w-6 h-6 bg-white border-2 border-slate-300 rounded-full flex items-center justify-center hover:border-slate-900 hover:shadow-md transition-all shadow-sm">
...
export default function EditorView({
      ...
      try {
          ...
          flowData.flow_data.triggers.forEach((trigger: any, index: number) => {
            ...
            reconstructedNodes.push({
              ...
              data: {
                ...
                triggerSubtitle: trigger.type === "whatsapp_followup"
                  ...
                  : trigger.type === "google_sheet"
                    ? config.sheet_name
                      ...
                      : config.sheet_id
                        ? `ID: ${config.sheet_id.substring(0, 15)}...`
                        : undefined
        ...
        if (flowData.flow_data?.nodes) {
          Object.entries(flowData.flow_data.nodes).forEach(([backendId, nodeData]: [string, any]) => {
            const uiPos = flowData.flow_data.ui_config?.nodes?.[backendId] || { x: 400 + Math.random() * 200, y: 300 + Math.random() * 200 };
            ...
            if (nodeData.type === "message") {
              nodeType = "action";
              const isWaFlow = flowData.flow_data?.triggers?.some((t: any) => t.type === "whatsapp_followup" || t.type === "google_sheet");
  ...
  const autoFit = () => {
    ...
    nodes.forEach((n) => { minX = Math.min(minX, n.x); maxX = Math.max(maxX, n.x + NODE_WIDTH); minY = Math.min(minY, n.y); maxY = Math.max(maxY, n.y + NODE_HEIGHT); });
  ...
  useEffect(() => {
    ...
    if (worldX < 0 || worldX > vw - NODE_WIDTH || worldY < 0 || worldY > vh - NODE_HEIGHT) autoFit();
  }, [nodes.length]);
  ...
  return (
    <div className="h-full w-full flex flex-col bg-slate-50 overflow-hidden relative font-sans">
      ...
      <div className="h-14 bg-white/95 backdrop-blur-sm border-b border-slate-200/60 flex items-center justify-between px-5 shrink-0 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2 text-[13px]">
            ...
            <input value={flowName} onChange={(e) => setFlowName(e.target.value)} className="text-slate-900 font-medium bg-transparent border-none outline-none focus:ring-0 w-36 px-1" placeholder="Untitled" />
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2">
            {saveStatus === "saved" ? (
              ...
              <span className="text-[13px] font-medium text-slate-600 flex items-center gap-1.5"><Loader2 className="w-4 h-4 animate-spin" />Saving</span>
            ) : (
              <button onClick={handleSave} className="text-[13px] text-slate-600 hover:text-slate-900 flex items-center gap-1.5 px-2 py-1 rounded-md hover:bg-slate-50 transition-all">
                <Save className="w-3.5 h-3.5" />Save
              </button>
            )}
            <div className="flex items-center border border-slate-200 rounded-lg overflow-hidden">
              <button className="p-1.5 hover:bg-slate-50 text-slate-400 hover:text-slate-600 disabled:opacity-30 transition-all" title="Undo" disabled><Undo2 className="w-3.5 h-3.5" /></button>
              <div className="w-px h-4 bg-slate-200" />
              <button className="p-1.5 hover:bg-slate-50 text-slate-400 hover:text-slate-600 disabled:opacity-30 transition-all" title="Redo" disabled><Redo2 className="w-3.5 h-3.5" /></button>
            </div>
          </div>
          <div className="h-5 w-px bg-slate-200" />
          <div className="flex items-center gap-2">
            <button onClick={handlePublish} disabled={saveStatus === "saving" || nodes.length === 0} className="flex items-center gap-1.5 px-3.5 py-1.5 bg-blue-600 hover:bg-slate-800 text-white rounded-lg text-[13px] font-medium disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-sm hover:shadow-md">
              {saveStatus === "saving" ? (<><Loader2 className="w-3.5 h-3.5 animate-spin" />Publishing</>) : (<><Play className="w-3.5 h-3.5" />Set Live</>)}
            </button>
            <button className="p-1.5 rounded-lg text-slate-400 hover:text-slate-600 hover:bg-slate-100 transition-all"><MoreVertical className="w-4 h-4" /></button>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 flex overflow-hidden relative">
        {/* LEFT SIDEBAR */}
        <aside className={`h-full bg-white border-r border-slate-200/60 shadow-sm transition-all duration-200 flex-shrink-0 ${sidebarOpen ? "w-60" : "w-0"}`} style={{ overflow: sidebarOpen ? "visible" : "hidden" }}>
                ...
                <div key={section.title} className="mb-5">
                  ...
                  <div className="space-y-0.5">
                    {section.steps.map((step) => (
                      <button key={step.id} type="button" onClick={() => addNodeByStepId(step.id)} className="w-full flex items-center gap-2.5 px-4 py-2.5 text-left hover:bg-slate-50 transition-all group border-l-2 border-transparent hover:border-slate-900">
                        <div className={`p-1.5 rounded-lg ${step.bgColor} group-hover:shadow-sm transition-shadow`}><StepIcon step={step} size="w-3.5 h-3.5" /></div>
                        <div className="min-w-0 flex-1">
        ...
        {/* SETTINGS SIDEBAR */}
        <div className={`absolute top-0 bottom-0 w-80 bg-white border-r border-slate-200/60 transition-all duration-200 shadow-xl ${selectedNodeId ? "z-30" : "z-10"} ${sidebarOpen ? "left-60" : "left-0"} ${selectedNodeId ? "translate-x-0" : "-translate-x-full"}`}>
          <SidebarSettings
          ...
          {nodes.length === 0 ? (
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              <div className="text-center max-w-md">
                ...
                <p className="text-slate-500 text-[13px] mb-6 leading-relaxed">A trigger is an event that starts your automation.<br />Click below to add your first trigger.</p>
                <button type="button" onClick={() => setShowTriggerModal(true)} className="inline-flex items-center gap-2 px-5 py-2.5 border-2 border-dashed border-slate-300 rounded-lg bg-white text-slate-700 font-medium text-[13px] hover:bg-slate-50 hover:border-slate-400 transition-all shadow-sm hover:shadow">
            ...
            <>
              <div className="absolute inset-0 pointer-events-none transition-transform duration-75 ease-out origin-top-left" style={{ transform: `translate(${pan.x}px, ${pan.y}px) scale(${zoom})` }}>
                <svg className="absolute inset-0 w-[10000px] h-[10000px]">
                  ...
                  {connections.map((conn, idx) => {
                    ...
                    return <path key={idx} d={getBezierPath(fromNode, toNode)} stroke="#94a3b8" strokeWidth="2" fill="none" markerEnd="url(#arrow)" className="transition-all" />;
                  })}
                </svg>
                {nodes.map((node) => (
                  <FlowStep
              ...
              <div className="absolute top-1/2 -translate-y-1/2 right-6 flex flex-col gap-3 items-center pointer-events-auto">
                <button onClick={() => { setActivePortId(null); setShowAddMenu(true); }} className="w-11 h-11 bg-slate-900 text-white rounded-xl shadow-lg flex items-center justify-center text-xl font-light hover:bg-slate-800 hover:shadow-xl transition-all hover:scale-105 active:scale-95">+</button>
                <div className="flex flex-col bg-white border border-slate-200 rounded-xl shadow-md overflow-hidden">
                  ...
                  <button onClick={() => setZoom((z) => Math.min(z + 0.1, 2))} className="w-9 h-9 flex items-center justify-center hover:bg-slate-50 text-slate-600 hover:text-slate-900 transition-all"><span className="text-lg font-medium">+</span></button>
                  <div className="w-full h-px bg-slate-200" />
                  <button onClick={() => setZoom((z) => Math.max(z - 0.1, 0.2))} className="w-9 h-9 flex items-center justify-center hover:bg-slate-50 text-slate-600 hover:text-slate-900 transition-all"><span className="text-lg font-medium">−</span></button>
                </div>
                <button className="w-9 h-9 bg-slate-700 text-white rounded-xl shadow-md flex items-center justify-center hover:bg-slate-800 transition-all hover:scale-105 active:scale-95" title="Help"><HelpCircle className="w-4 h-4" /></button>
              </div>
            </>
          )}
        </div>
