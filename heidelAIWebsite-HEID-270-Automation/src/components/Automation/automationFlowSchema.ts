...
export interface AutomationFlowDetails {
  flow_id?: string; // Backend uses flow_id, not _id!
  name: string;
  description?: string;
...
function inferTriggerType(nodes: EditorNode[]): TriggerTypeEnum {
  ...
  if (t === 'trigger_comment' || t === 'instagram_post_comment') return 'instagram_comment';
  if (t === 'instagram_comment') return 'instagram_comment';
