...
export type TriggerTypeEnum =
  ...
  | 'default'
  | 'whatsapp_followup'
  | 'whatsapp_message_received'  // WA bot flows — handled by WaBotEditor
  | 'google_sheet';
...
function inferTriggerType(nodes: EditorNode[]): TriggerTypeEnum {
  ...
  const stepId = trigger?.data?.stepId as string | undefined;

  if (t === 'google_sheet' || stepId === 'trigger_google_sheet')               return 'google_sheet';
  if (t === 'whatsapp_followup' || stepId === 'trigger_whatsapp_followup')     return 'whatsapp_followup';
  if (t === 'whatsapp_message_received' || stepId === 'trigger_whatsapp_message') return 'whatsapp_message_received';
  if (t === 'trigger_comment' || t === 'instagram_comment')                    return 'instagram_comment';
  if (t === 'trigger_story_reply' || t === 'story_reply')                      return 'story_reply';
  if (t === 'trigger_instagram_message' || t === 'instagram_message')          return 'instagram_message';
