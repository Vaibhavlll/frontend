...
export default function SidebarSettings({
  ...
  const {
    ...
    loading: loadingWhatsAppConversations,
    isFetchingMore,
    hasMore,
    loadMore,
    ...
    initialFilters: { platform: ["whatsapp"] },
    limit: 20,
  });

  const observerTarget = useRef<HTMLDivElement>(null);

  // Observes the last item for triggering refetch when user reaches end of current items
  useEffect(() => {
    const observer = new IntersectionObserver(
      (entries) => {
        // If the target is visible, and we aren't currently loading, fetch more
        // Cap total at 50 as per requirement
        if (
          entries[0].isIntersecting &&
          hasMore &&
          !loadingWhatsAppConversations &&
          !isFetchingMore &&
          whatsappConversations.length < 50
        ) {
          loadMore();
        }
      },
        ...
        {isWhatsAppFollowUpTrigger && (
          <div className="space-y-4">
            ...
            <div>
              ...
              {loadingWhatsAppConversations && whatsappConversations.length === 0 ? (
                <div className="flex justify-center py-8">
                ...
                <div className="border border-slate-200 rounded-lg overflow-hidden max-h-96 overflow-y-auto ios-scrollbar">
                  ...
                  {hasMore && whatsappConversations.length < 50 && (
                    <div
                      ref={observerTarget}
                      className="w-full py-4 flex justify-center items-center"
                    >
                      {(loadingWhatsAppConversations || isFetchingMore) && (
