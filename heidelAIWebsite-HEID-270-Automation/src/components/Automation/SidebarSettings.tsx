...
interface WhatsAppConversation {
  // Backend returns 'id' from /api/whatsapp/conversations endpoint
  // but stores 'conversation_id' internally — handle both
  id?: string;
...
// Helper: get the actual conversation ID regardless of which field the backend returns
...
export default function SidebarSettings({
  ...
  useEffect(() => {
    if (
      !node ||
      node.type !== "trigger" ||
      (node.data?.triggerType !== "whatsapp_followup" && node.data?.stepId !== "trigger_whatsapp_followup")
    ) {
  ...
  useEffect(() => {
    ...
    // Don't load Instagram posts for WhatsApp triggers
    if (node.data?.triggerType === "whatsapp_followup" || node.data?.stepId === "trigger_whatsapp_followup") {
      setTriggerPosts([]);
      return;
    }
  ...
  return (
    <div className="flex flex-col h-full bg-white">
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <textarea
                ...
                placeholder={`Enter your follow-up message...\n\nExample:\nHi {{customer_name}}, just checking in! Are you still interested in our product?`}
                value={node.data?.text ?? ""}
                onChange={(e) => update({ text: e.target.value })}
              />
              {/* FIX: Correct variable syntax is {{customer_name}}, NOT {{trigger_data.customer_name}} */}
            ...
            <div>
              ...
              <div className="space-y-2">
                {[
                  ...
                  {
                    label: "Limited Offer",
                    text: "Hi {{customer_name}}, we have a special offer expiring soon just for you. Would you like to know more?",
                  },
                  {
        ...
        {isTrigger && !isWhatsAppFollowUpTrigger && (
          <div className="space-y-4">
            <div className="flex items-center gap-2">
            ...
            {node.data?.logicType === "delay" && (
              <div className="p-4 rounded-xl border border-amber-200 bg-amber-50 space-y-3">
                ...
                <div className="flex gap-2 items-center">
                  ...
                  />
                  {/* FIX: Added 'days' option — required for WhatsApp follow-up scheduling */}
                  <select
