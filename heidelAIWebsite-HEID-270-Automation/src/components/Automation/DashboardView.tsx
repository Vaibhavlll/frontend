...
import { isWaBotFlow } from "./AutomationHub"; // shared detector

interface AutomationFlow {
  _id?: string;
  flow_id: string;
  name: string;
  // "active" is the DB value WaBotEditor publishes; we display it as "Live"
...
interface DashboardViewProps {
  onCreateNew: () => void;
  // flowData is the full raw document so AutomationHub can detect the editor
...
export default function DashboardView({
  ...
  // ── Which API base path to use for a given flow ─────────────────────────
  // WaBot flows live in wa_bot_flows_{org_id}, editor flows in automation_flows_{org_id}
  ...
  const handleDeleteSubmit = async () => {
    ...
    try {
      ...
      else if (err.response?.status === 404) errorMessage = "Flow not found. It may have already been deleted.";
      else if (err.response?.status === 403) errorMessage = "You do not have permission to delete this flow";
  ...
  const handleUnpublish = async (flow: AutomationFlow) => {
    ...
    try {
      ...
      else if (err.response?.status === 404) errorMessage = "Flow not found";
      else if (err.response?.status === 400) errorMessage = "Flow is already in draft mode";
      toast.error(errorMessage);
    }
  };
  ...
  const handleDuplicate = async (flow: AutomationFlow) => {
    ...
    try {
      ...
      // Strip identity fields so the backend treats this as a new document
  ...
  // ── FILTER & STATS ───────────────────────────────────────────────────────
  // "active" (WaBot published) is shown as "Live" alongside "published"
  ...
  const filteredFlows = flows.filter((flow) => {
    ...
    const matchesFilter =
      ...
      // "published" filter should catch "active" WaBot flows too
      ...
      <div className="relative bg-gradient-to-b from-slate-50 to-white border-b border-slate-200/60 px-6 py-6">
        ...
        <div className="grid grid-cols-4 gap-4">
          <div className="relative group">
            ...
            <div className="relative bg-white rounded-xl p-4 border border-slate-200/60 shadow-sm hover:shadow-md transition-all">
              <div className="flex items-center justify-between mb-3">
                <span className="text-[11px] font-semibold text-slate-600 uppercase tracking-wider">Total</span>
                <div className="p-1.5 rounded-lg bg-slate-50"><Zap className="w-4 h-4 text-slate-600" /></div>
              </div>
              <p className="text-2xl font-semibold text-slate-900 mb-1">{stats.total}</p>
          ...
          <div className="relative group">
            ...
            <div className="relative bg-white rounded-xl p-4 border border-slate-200/60 shadow-sm hover:shadow-md transition-all">
              <div className="flex items-center justify-between mb-3">
                <span className="text-[11px] font-semibold text-slate-600 uppercase tracking-wider">Live</span>
                <div className="p-1.5 rounded-lg bg-emerald-50"><Play className="w-4 h-4 text-emerald-600" /></div>
              </div>
              <p className="text-2xl font-semibold text-slate-900 mb-1">{stats.live}</p>
          ...
          <div className="relative group">
            ...
            <div className="relative bg-white rounded-xl p-4 border border-slate-200/60 shadow-sm hover:shadow-md transition-all">
              <div className="flex items-center justify-between mb-3">
                <span className="text-[11px] font-semibold text-slate-600 uppercase tracking-wider">Draft</span>
                <div className="p-1.5 rounded-lg bg-amber-50"><Pause className="w-4 h-4 text-amber-600" /></div>
              </div>
              <p className="text-2xl font-semibold text-slate-900 mb-1">{stats.draft}</p>
          ...
          <div className="relative group">
            ...
            <div className="relative bg-white rounded-xl p-4 border border-slate-200/60 shadow-sm hover:shadow-md transition-all">
              <div className="flex items-center justify-between mb-3">
                <span className="text-[11px] font-semibold text-slate-600 uppercase tracking-wider">Archived</span>
                <div className="p-1.5 rounded-lg bg-slate-50"><Archive className="w-4 h-4 text-slate-600" /></div>
              </div>
              <p className="text-2xl font-semibold text-slate-900 mb-1">{stats.archived}</p>
                ...
                {(["all", "published", "draft", "archived"] as const).map((status) => (
                  <button
                    ...
                    className={`w-full text-left px-3.5 py-2 text-[13px] font-medium transition-colors ${
                      filterStatus === status ? "bg-slate-900 text-white" : "text-slate-700 hover:bg-slate-50"
                    }`}
                  >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredFlows.map((flow) => (
              <FlowCard
                key={flow.flow_id}
                flow={flow}
                // Pass the full flow document so AutomationHub can detect the editor
              ...
              {showMenu && (
                <>
                  ...
                  <div className="absolute right-0 mt-2 w-44 bg-white border border-slate-200 rounded-lg shadow-2xl z-20 py-1 animate-in fade-in slide-in-from-top-2 duration-200">
                    ...
                    {isLive && (
                      <button
                        onClick={(e) => { e.stopPropagation(); onUnpublish(); setShowMenu(false); }}
                        className="w-full text-left px-3 py-1.5 hover:bg-slate-50 text-[12px] font-medium text-slate-700 transition-colors flex items-center gap-2 group/item"
                      >
