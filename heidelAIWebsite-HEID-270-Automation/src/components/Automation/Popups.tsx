...
return (
  <>
    {/* Backdrop */}
    <div 
      className="fixed inset-0 bg-slate-900/10 backdrop-blur-[2px] z-[999]"
      onClick={onClose}
    />
    
    {/* Right Sidebar Panel */}
    <div className="fixed right-0 top-0 h-full w-[400px] bg-white shadow-2xl z-[1000] animate-in slide-in-from-right duration-200 overflow-y-auto border-l border-slate-200/60">
      <div className="sticky top-0 bg-white/95 backdrop-blur-sm border-b border-slate-200/60 px-5 py-4 flex items-center justify-between z-10">
        <h2 className="text-[17px] font-semibold text-slate-900">{isFirstStep ? 'Choose First Step' : 'Choose Next Step'}</h2>
        <button 
          onClick={onClose}
      ...
      <div className="p-5 space-y-6">
        {STEP_SECTIONS.map((section) => (
          <div key={section.title}>
            <h3 className="text-[13px] font-semibold text-slate-900 mb-3">{section.title}</h3>
            <div className="space-y-2">
              {section.steps.map((step) => (
                <button
                  key={step.id}
                  onClick={() => handleStepClick(step.id)}
                  className="w-full px-4 py-3 border-2 border-dashed border-slate-200 rounded-lg hover:border-slate-300 hover:bg-slate-50 transition-all group text-left flex items-center gap-3 shadow-sm hover:shadow"
                >
                  <div className={`p-1.5 rounded-lg shrink-0 ${step.bgColor} group-hover:shadow-sm transition-shadow`}>
                    <StepIcon step={step} size="w-4 h-4" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-[13px] font-semibold text-slate-900 group-hover:text-slate-700 transition-colors flex items-center gap-2">
                      {step.label}
                      {(step as { pro?: boolean }).pro && (
                        <span className="px-1.5 py-0.5 bg-slate-900 text-white text-[10px] font-semibold rounded uppercase tracking-wide">PRO</span>
                      )}
                    </p>
...
export function TriggerSelectionModal({ onClose, onSelectTrigger }: TriggerSelectionModalProps) {
  ...
  return (
  <div className="fixed inset-0 bg-slate-900/20 backdrop-blur-sm z-[1000] flex items-center justify-center p-6 animate-in fade-in duration-200">
    <div className="bg-white w-full max-w-3xl rounded-2xl shadow-2xl flex flex-col max-h-[85vh] overflow-hidden border border-slate-200/60">
      <div className="px-6 py-4 border-b border-slate-200/60 flex items-center justify-between gap-4 shrink-0 bg-slate-50/50">
        <h2 className="text-[17px] font-semibold text-slate-900">Start automation when...</h2>
        <div className="relative flex-1 max-w-xs">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2 border border-slate-200 rounded-lg text-[13px] focus:outline-none focus:ring-2 focus:ring-slate-900/10 focus:border-slate-900 transition-all shadow-sm"
          />
        </div>
        <button
          onClick={onClose}
          className="w-8 h-8 rounded-lg hover:bg-slate-100 flex items-center justify-center text-slate-400 hover:text-slate-700 transition-all shrink-0"
        >
          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
          </svg>
      ...
      <div className="flex flex-1 min-h-0">
        <aside className="w-44 border-r border-slate-200/60 p-3 space-y-0.5 shrink-0 bg-slate-50/50">
          {TRIGGER_SECTIONS.map((section) => (
            ...
            >
              {section.title === 'Instagram' ? (
                ...
                <span className="w-4 h-4 rounded-full bg-slate-200 flex items-center justify-center text-[10px] shrink-0">👤</span>
              )}
              {section.title}
            </button>
          ))}
        ...
        <main className="flex-1 overflow-y-auto p-5 bg-white">
          <h3 className="text-[11px] font-semibold text-slate-600 uppercase tracking-wider mb-1">Triggers</h3>
          <p className="text-[13px] text-slate-500 mb-4">Select the event that starts your automation</p>
          <div className="space-y-2">
            {filteredSteps.map((step) => (
              <button
                ...
                className="w-full flex items-center gap-4 p-4 rounded-xl border-2 border-slate-200 hover:border-slate-900 hover:bg-slate-50 transition-all text-left group shadow-sm hover:shadow-md"
              >
                <div className={`p-2 rounded-xl shrink-0 ${step.bgColor} group-hover:shadow-sm transition-shadow`}>
                  <StepIcon step={step} size="w-5 h-5" />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-[14px] font-semibold text-slate-900 group-hover:text-slate-700 flex items-center gap-2">
                    {step.label}
                    {(step as { pro?: boolean }).pro && (
                      <span className="px-1.5 py-0.5 bg-slate-900 text-white text-[10px] font-semibold rounded uppercase tracking-wide">PRO</span>
                    )}
                  </p>
                  <p className="text-[12px] text-slate-500 mt-0.5 leading-relaxed">{step.description}</p>
                </div>
              </button>
            ))}
          </div>
          {filteredSteps.length === 0 && (
            <p className="text-[13px] text-slate-500 py-8 text-center">No triggers match your search.</p>
          )}
        </main>
      </div>
    </div>
