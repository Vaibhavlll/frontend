...
export function validateFlowBeforeSave(
  ...
  if (nodes.length === 0) { errors.push('Flow has no nodes.'); return { valid: false, errors, warnings }; }

  const triggerNodes = nodes.filter((n) => n.type === 'trigger');
  if (triggerNodes.length === 0) errors.push('Flow must have at least one Trigger node.');

  const isBotFlow = isInteractiveBotFlow(nodes);

  if (isBotFlow) {
    const hasWelcome = nodes.some((n) => n.data?.stepId === 'wa_welcome_message');
    if (!hasWelcome) warnings.push('Interactive bot flows should have a Welcome Message node as the starting node.');

    for (const node of nodes) {
      const stepId = node.data?.stepId;
      if (stepId === 'wa_welcome_message' || stepId === 'wa_interactive_button') {
        if (!node.data?.waBodyText) errors.push(`"${node.label}" must have body text.`);
        if (!node.data?.waButtons?.length) errors.push(`"${node.label}" must have at least one button.`);
        for (const btn of node.data?.waButtons || []) {
  ...
  // Google Sheet flow
  const isGSFlow = triggerNodes.some((t) => t.data?.triggerType === 'google_sheet' || t.data?.stepId === 'trigger_google_sheet');
  if (isGSFlow) {
    for (const t of triggerNodes) {
      if (!t.data?.triggerConfig?.sheet_id) errors.push('Google Sheet trigger must have a sheet selected.');
      if (!t.data?.triggerConfig?.column_name) errors.push('Google Sheet trigger must have a phone number column selected.');
    }
    if (!nodes.some((n) => n.data?.stepId === 'content_whatsapp_message')) errors.push('Google Sheet flow must include a WhatsApp Message node.');
    return { valid: errors.length === 0, errors, warnings };
  }

  // WA followup flow
  const isWAFlow = triggerNodes.some((t) => t.data?.triggerType === 'whatsapp_followup' || t.data?.stepId === 'trigger_whatsapp_followup');
  if (isWAFlow) {
    for (const t of triggerNodes) {
      if (!t.data?.triggerConfig?.conversation_id) errors.push('WhatsApp Follow-Up trigger must have a conversation selected.');
    }
    if (!nodes.some((n) => n.type === 'logic' && n.data?.logicType === 'delay')) errors.push('WhatsApp Follow-Up flow must include a Smart Delay node.');
    if (!nodes.some((n) => n.data?.stepId === 'content_whatsapp_message')) errors.push('WhatsApp Follow-Up flow must include a WhatsApp Message node.');
    return { valid: errors.length === 0, errors, warnings };
  }
  ...
  for (const node of nodes) {
    if (node.type === 'logic' && node.data.logicType === 'condition') {
      if (edges.filter((e) => e.from === node.id).length < 2) errors.push(`Condition node "${node.label}" must have both True and False connections.`);
    }
    const stepId = node.data?.stepId;
    if (stepId === 'action_send_dm' && !node.data.text) errors.push(`Send DM "${node.label}" must have message text.`);
    if (stepId === 'action_reply_comment' && !node.data.text) errors.push(`Reply to Comment "${node.label}" must have reply text.`);
    if (stepId === 'action_tag' && !node.data.tagName) errors.push(`Add Tag "${node.label}" must have a tag name.`);
  }
