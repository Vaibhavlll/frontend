...
export interface FlowNodePayload {
  ...
  // Preserved so the node renderer can show "WhatsApp Message" vs "Send Message"
  app?: string;           // 'whatsapp' | 'instagram'
  actionType?: string;    // 'whatsapp_message' | 'instagram_message'
  name: string;
...
export function exportFlowToJSON(
  ...
  for (const node of nodes) {
    ...
    if (node.type === 'trigger') {
      ...
      } else if (triggerType === 'google_sheet' || triggerType === 'trigger_google_sheet') {
    ...
    if (stepId === 'content_whatsapp_message') {
      ...
      flowDocument.nodes[node.id] = {
        ...
        // config.content.text — primary path read by backend execution engine
        config: {
          content: {
            text: messageText,
          },
        },
        // content.text — fallback path for any existing readers
        content: {
          text: messageText,
        },
      };
    ...
    if (stepId === 'action_reply_comment') {
      ...
      if (includeUiConfig) flowDocument.ui_config!.nodes[node.id] = { x: node.x, y: node.y };
      continue;
    }
...
export function validateFlowBeforeSave(
  ...
  // ─── GOOGLE SHEET FLOW VALIDATION ────────────────────────────────────────
  const isGoogleSheetFlow = triggerNodes.some(
    (t) => t.data?.triggerType === 'google_sheet' || t.data?.stepId === 'trigger_google_sheet'
  );

  if (isGoogleSheetFlow) {
    for (const trigger of triggerNodes) {
      if (
        ...
        if (rowStart !== undefined && rowEnd !== undefined) {
          if (rowStart < 1) errors.push('Row range start must be at least 1.');
          if (rowEnd < rowStart) errors.push('Row range end must be greater than or equal to start.');
        }
      }
    }

    const hasWaMessage = nodes.some((n) => n.data?.stepId === 'content_whatsapp_message');
    if (!hasWaMessage) {
  ...
  const isWhatsAppFlow = triggerNodes.some(
    (t) =>
      t.data?.triggerType === 'whatsapp_followup' ||
      t.data?.stepId === 'trigger_whatsapp_followup'
  );
  ...
  if (isWhatsAppFlow) {
    ...
    const hasDelay = nodes.some((n) => n.type === 'logic' && n.data?.logicType === 'delay');
    if (!hasDelay) {
  ...
  for (const trigger of triggerNodes) {
    ...
    if (triggerType === 'instagram_comment' || triggerType === 'trigger_comment') {
      const hasKeyword =
        trigger.data.triggerConfig?.keyword || trigger.data.triggerConfig?.keywords;
