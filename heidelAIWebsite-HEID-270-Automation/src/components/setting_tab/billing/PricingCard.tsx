...
export function PricingCard({ item, paymentFrequency }: PricingCardProps) {
    const basePrice = parseInt(item.price.replace('₹', '')) || 0;

    // Check if discount applies
    const isYearlyGrowth = paymentFrequency === 'annually' && item.id === 'growth';
  ...
  const price = finalPriceString
  const isHighlighted = item.highlighted
  const isPopular = false // Assuming no popular flag in the item data structure
  // Modified to check for enterprise to trigger the contact drawer
  const isEnterprise = item.title.toLowerCase().includes("enterprise")
    ...
    >
      ...
      <div className="relative z-10 flex flex-col gap-8 h-full">
          ...
          {typeof price === "number" ? (
            <>
              <NumberFlow
                format={{
                  style: "currency",
                  currency: "USD", // Updated to USD to match your image
        ...
        <div className="flex-1 space-y-3 sm:space-y-4">
          <h3 className="text-sm sm:text-base font-medium">{item.description}</h3>
            ...
            <Drawer.Portal>
              ...
              <Drawer.Content className="bg-white flex flex-col fixed bottom-0 left-0 right-0 max-h-[85vh] rounded-t-[10px] z-[999]">
                <div className="max-w-md flex justify-center flex-col w-full mx-auto overflow-auto p-4 mb-4 rounded-t-[10px]">
                  ...
                  <form method="POST" onSubmit={handleDemoRequest}>
                    {/* Your form fields remain exactly the same */}
                    <label htmlFor="name" className="font-medium text-gray-900 text-sm mt-8 mb-2 block">Name</label>
                    <input value={userName} onChange={(e) => setUserName(e.target.value)} id="name" name="entry.545040562" type="text" className="border border-gray-200 bg-white w-full text-sm p-3 py-2 h-9 rounded-xl outline-none focus:ring-2 focus:ring-black/5 text-gray-900" />
                    
                    <label htmlFor="email" className="font-medium text-gray-900 text-sm mt-4 mb-2 block">Email ID</label>
                    <input value={userEmail} onChange={(e) => setUserEmail(e.target.value)} type="text" name="entry.1231199003" className="border border-gray-200 bg-white w-full resize-none rounded-xl p-3 py-2 text-gray-900 outline-none focus:ring-2 focus:ring-black/5 focus:ring-offset-0" />
                    
                    <label htmlFor="number" className="font-medium text-gray-900 text-sm mt-4 mb-2 block">Phone Number</label>
                    <input value={userPhone} onChange={(e) => { const value = e.target.value.replace(/[^0-9\s\-\(\)\+]/g, ''); setUserPhone(value); }} type="tel" name="entry.1154213412" placeholder="+91 9876543210" className="border border-gray-200 bg-white w-full resize-none rounded-xl p-3 py-2 text-gray-900 outline-none focus:ring-2 focus:ring-black/5 focus:ring-offset-0" />
                    
                    <label htmlFor="company" className="font-medium text-gray-900 text-sm mt-4 mb-2 block">Company name</label>
                    <input value={companyName} onChange={(e) => setCompanyName(e.target.value)} type="text" name="entry.1081121699" className="border border-gray-200 bg-white w-full resize-none p-3 rounded-xl py-2 text-gray-900 outline-none focus:ring-2 focus:ring-black/5 focus:ring-offset-0" />
                    
                    <p className="text-neutral-600 text-xs mt-2 mb-8">
                      By submitting you agree to our{" "}
                      <button type="button" className="underline cursor-pointer" onClick={() => router.push("/terms-of-service")}>Terms & Conditions.</button>
                    </p>                    
                    <button type="submit" className="h-[44px] bg-black mx-auto rounded-full sticky bottom-0 text-gray-50 mt-4 w-full font-medium">Submit</button>
                  </form>
                </div>
              </Drawer.Content>
            </Drawer.Portal>
...
// --- YOUR CUSTOM PRICING CARD COMPONENT (Slightly modified to match "Enterprise" logic) ---
